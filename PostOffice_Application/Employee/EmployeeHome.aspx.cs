        ...
        protected void btnGetDateRangeReport_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        con.Open();
                        string getPackageInfoQuery = "SELECT SHIPMENT.Tracking_Num AS [Tracking Number], SHIPMENT.Package_Type, SHIPMENT.Contents, SHIPMENT.Value_Of_Contents AS Value, DELIVERY_STATUS.Date_Shipped, concat(Address.Street_Address1, ' ', Address.Street_Address2, ' ', Address.Apartment_Num, ' ', Address.City, ', ', STATES.State_Name, ' ', Address.Zip, ', ', COUNTRY.Country_Name) AS Address FROM SHIPMENT LEFT JOIN Address ON SHIPMENT.Recipient_Address_ID = Address.Address_ID LEFT JOIN COUNTRY ON Address.Country_ID = Country.Country_ID LEFT JOIN STATES ON Address.State_ID = STATES.State_ID LEFT JOIN DELIVERY_STATUS ON SHIPMENT.Delivery_Status = DELIVERY_STATUS.Delivery_Status_ID WHERE NOT SHIPMENT.Delivery_Status = 1 AND NOT SHIPMENT.Delivery_Status = 2 AND DELIVERY_STATUS.Date_Shipped BETWEEN @startDate AND @endDate;";
